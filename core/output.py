...
def linkedin2usernames(users,out_dir):
    ...
    out_dir = f'{out_dir}/linkedin_userames'
...
def write_files(out_dir,name_list):
    ...
    files = {'rawnames': open(f'{out_dir}/rawnames.txt', 'w')}
    files['f.last'] = open(f'{out_dir}/f.last.txt', 'w')
    files['flast'] = open(f'{out_dir}/flast.txt', 'w')
    files['firstl'] = open(f'{out_dir}/firstl.txt', 'w')
    files['firstlast'] = open(f'{out_dir}/first.last.txt', 'w')
    files['fonly'] = open(f'{out_dir}/first.txt', 'w')
    files['lastf'] = open(f'{out_dir}/lastf.txt', 'w')
    ...
    for name in name_list:
        ...
        try:
            if len(parse) > 2:
                ...
                files['f.last'].write(f'{first[0]}.{second}' + '\n')
                files['f.last'].write(f'{first[0]}.{third}' + '\n')
                files['lastf'].write(second + first[0] + '\n')
                files['lastf'].write(third + first[0] + '\n')
                files['firstlast'].write(f'{first}.{second}' + '\n')
                files['firstlast'].write(f'{first}.{third}' + '\n')
                ...
                files['f.last'].write(f'{first[0]}.{last}' + '\n')
                files['lastf'].write(last + first[0] + '\n')
                files['firstlast'].write(f'{first}.{last}' + '\n')
                files['firstl'].write(first + last[0] + '\n')
            files['fonly'].write(first + '\n')
        except IndexError:
            print(f"{colors.info} Struggled with this tricky name: '{name}'.{colors.end}")
...
def saveEmails(out_dir,results):
    ...
    out_dir = f'{out_dir}/emails'
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    file = f'{out_dir}/socialpwned_emails.txt'
...
def saveResultsPwnDB(out_dir,results):
    ...
    out_dir = f'{out_dir}/pwndb'
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    file = f'{out_dir}/socialpwned_pwndb.txt'
    with open(f'{out_dir}/passwords_pwndb.txt', 'w') as passwords_file:
        pwndb_file = open(f'{out_dir}/pwndb.txt', 'w')
...
def saveResultsDehashed(out_dir,results):
    ...
    out_dir = f'{out_dir}/dehashed'
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    file = f'{out_dir}/socialpwned_dehashed.txt'
    with open(f'{out_dir}/raw_dehashed.txt', 'w') as raw_file:
        with open(file, "w") as resultFile:
            for result in results:
                for item in result:
                    ...
                    print(
                        f"{colors.good} Email: {colors.V}{email}{colors.B} Password: {colors.V}{password}{colors.B} Hashed Password: {colors.V}{hashed_password}{colors.B} Source: {colors.V}{database_name}{colors.end}"
                    )

                    resultFile.write(f"{email}:{password}:{hashed_password}" + "\n")
...
def saveResultsInstagram(out_dir,results):
    ...
    out_dir = f'{out_dir}/instagram'
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    file = f'{out_dir}/socialpwned_instagram.txt'
    with open(file, "w") as resultFile:
        for result in results:
            item = json.loads(result)
            user = item.get("user")
            email = item.get("email")
            resultFile.write(f"{user}:{email}" + "\n")
    resultFile.close()

def saveResultsTwitter(out_dir,results):

    out_dir = f'{out_dir}/twitter'
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    file = f'{out_dir}/socialpwned_twitter.txt'
    with open(file, "w") as resultFile:
        for result in results:
            item = json.loads(result)
            user = item.get("user")
