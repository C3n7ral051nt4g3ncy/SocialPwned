...
def getCompanyInformation(api,companyID):
    ...
    for location in locations:
        ...
        print(
            f"{colors.good} Country: {colors.W}{country}{colors.B} Area: {colors.W}{area}{colors.B} City: {colors.W}{city}{colors.B} Postal Code: {colors.W}{postalCode}{colors.end}"
        ) 
...
def getEmailsFromUsers(api,employees):
    ...
    for employee in employees:
        ...
        print(
            f"{colors.good} User ID: {colors.W}{userID}{colors.B} Public ID of employee: {colors.W}{employeeID}{colors.B} Email: {colors.W}{email}{colors.B} Phone: {colors.W}{phone}{colors.B} Twitter: {colors.W}{twitter}{colors.end}"
        )
...
def unionUsers(targets):
    ...
    for target in targets:
        results.extend(iter(target))
    print(
        f"{colors.info} {len(results)} different users have been found in total ^-^{colors.end}"
    )
...
def searchUsers(api,query):
    ...
    for item in items:
        ...
        print(
            f"{colors.good} User: {colors.W}{user}{colors.B} userID: {colors.W}{userID}{colors.end}"
        )

    print(f"{colors.info} {len(items)} users have been found ^-^{colors.end}")
    return items

def searchCompanies(api,query):
    print(f"{colors.info} Searching companies... :){colors.end}")
    query = query.replace(',','')
    ...
    for item in items:
        ...
        print(
            f"{colors.good} Name: {colors.W}{nameCompany}{colors.B} company ID: {colors.W}{companyID}{colors.B} Number of employees: {colors.W}{numberEmployees}{colors.end}"
        )

    print(f"{colors.info} {len(items)} companies have been found ^-^{colors.end}")
    return items

def searchUsersOfCompany(api,nameCompany):
    print(
        f"{colors.info} Searching employees of company: {nameCompany}{colors.end}"
    )

    employees = api.search_people(keywords=nameCompany)
    print(
        f"{colors.info} {len(employees)} employees have been found ^-^{colors.end}"
    )
...
def getUserInformation(api,publicID):
    ...
    phone = str(info.get("phone"))
    print(
        f"{colors.good} User: {colors.W}{publicID}{colors.B} Email: {colors.W}{email}{colors.B} Phone: {colors.W}{phone}{colors.B} Twitter: {colors.W}{twitter}{colors.end}"
    )
       

    id_json = publicID if email == "Not Found" else email
...
def getContactInformation(api,publicID):
    ...
    print(f"{colors.info} Searching user contact information: {publicID}")
    info = api.get_profile_contact_info(publicID)
    email = ''
    twitter = ''
    phone = ''
    if info.get("email_address") is None:
        email = "Not Found"
    else:
...
def sendContactRequest(api, userID):
    ...
    if response == 201:
        print(f"{colors.good} Contact request successfully{colors.end}")
    else:
        print(f"{colors.bad} Contact request unsuccessfully{colors.end}")

    return response

def getFollowers(api, userID):
    print(f"{colors.info} Getting contacts...{colors.end}")
    followers = api.get_profile_connections(userID)
    print(
        f"{colors.info} {len(followers)} contacts have been found ^-^{colors.end}"
    )

    return followers

def getMyContacts(api):
    print(f"{colors.info} Getting your contacts...{colors.end}")
