    ...
    if isfile(config.data_path):
        try:
            with open(config.data_path, 'r') as f:
                ...
                cookies = out["cookies"]
                print(f"{colors.good} Detected stored cookies, checking it{colors.end}")
                return cookies
        except Exception:
            print(f"{colors.bad} Stored cookies are corrupted{colors.end}")
            return False
    print(f"{colors.info} No stored cookies found{colors.end}")
    return False
...
def get_hangouts_tokens(driver, cookies, tmprinter):
    ...
    tmprinter.out(f"{colors.info} Fetching Hangouts homepage...{colors.end}")
    ...
    try:
        ...
        tmprinter.out(f"{colors.info} Request found !{colors.end}")
        driver.close()
        tmprinter.out("")
    except SE_TimeoutExepction:
        tmprinter.out("")
        exit(
            f"{colors.bad} Selenium TimeoutException has occured. Please check your internet connection, proxies, vpns, et cetera.{colors.end}"
        )
...
def get_internal_tokens(driver, cookies, tmprinter):
    ...
    tmprinter.out(f"{colors.info} Fetching Drive homepage...{colors.end}")
    ...
    tmprinter.out(
        f"{colors.info} Waiting for the authorization header, it can takes a few minutes...{colors.end}"
    )
...
def check_and_gen(ghunt_SID,ghunt_SSID,ghunt_APISID,ghunt_SAPISID,ghunt_HSID):
    ...
    if not cookies_from_file:
        ...
        print(f"{colors.good} Cookies obtained from the json file{colors.end}")
    else:
        # in case user wants to enter new cookies (example: for new account)
        html = get_authorization_source(cookies_from_file)
        valid = False
        if html:
            print(f"{colors.good} The cookies seems valid !{colors.end}")
            valid = True
        else:
            print(f"{colors.bad} eems like the cookies are invalid.{colors.end}")
            exit(f"{colors.bad} Please put valid cookies. Exiting... {colors.end}")
    ...
    if new_cookies_entered or not cookies_from_file:
        ...
        if html:
            ...
            exit(
                f"{colors.bad} Seems like the cookies are invalid, try regenerating them.{colors.end}"
            )
    ...
    print(f"{colors.info} Extracting the tokens...{colors.end}")
    ...
    if trigger not in html:
        ...
        print(f"{colors.info} Google Docs Token => {gdoc_token}{colors.end}")

    # Extracting Internal People API tokens
    internal_auth, internal_token = get_internal_tokens(driver, cookies, tmprinter)
    print(f"{colors.info} Internal APIs Token => {internal_token}{colors.end}")
    print(
        f"{colors.info} Internal APIs Authorization => {internal_auth}{colors.end}"
    )


    # Extracting Hangouts tokens
    auth_token, hangouts_token = get_hangouts_tokens(driver, cookies, tmprinter)
    print(f"{colors.info} Hangouts Authorization => {auth_token}{colors.end}")
    print(f"{colors.info} Hangouts Token => {hangouts_token}{colors.end}")
